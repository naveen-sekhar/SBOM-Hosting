...
# === Function: Run Syft & Grype Analysis ===
run_combined_sbom_analysis() {
  read -p "Enter folder OR image name to scan: " TARGET
  read -p "Enter output directory: " OUTPUT_DIR
  read -p "Enter a name for your report files (no spaces): " NAME
  ...
  mkdir -p "$OUTPUT_DIR/sbom_outputs"/{spdx_comparison_reports,cyclonedx_comparison_reports}
  cd "$OUTPUT_DIR/sbom_outputs" || exit 1
  ...
  echo "🔄 Generating SBOMs using Syft..."
  if [[ "$SCAN_MODE" == "image" ]]; then
    syft "$TARGET" -o spdx-json > "syft_sbom_spdx_${NAME}.json"
    syft "$TARGET" -o cyclonedx-json > "syft_sbom_cyclonedx_${NAME}.json"
  else
    syft dir:"$TARGET" -o spdx-json > "syft_sbom_spdx_${NAME}.json"
    syft dir:"$TARGET" -o cyclonedx-json > "syft_sbom_cyclonedx_${NAME}.json"
  fi

  echo "🔄 Generating SBOMs using Trivy..."
  if [[ "$SCAN_MODE" == "image" ]]; then
    trivy image --format spdx-json --output "trivy_sbom_spdx_${NAME}.json" "$TARGET"
    trivy image --format cyclonedx --output "trivy_sbom_cyclonedx_${NAME}.json" "$TARGET"
  else
    trivy fs --format spdx-json --output "trivy_sbom_spdx_${NAME}.json" "$TARGET"
    trivy fs --format cyclonedx --output "trivy_sbom_cyclonedx_${NAME}.json" "$TARGET"
  fi

  echo "📊 Counting components..."
  echo -n "  SPDX (Syft): "
  jq '.packages | length' "syft_sbom_spdx_${NAME}.json"
  echo -n "  CycloneDX (Syft): "
  jq '.components | length' "syft_sbom_cyclonedx_${NAME}.json"
  echo -n "  SPDX (Trivy): "
  jq '.packages | length' "trivy_sbom_spdx_${NAME}.json"
  echo -n "  CycloneDX (Trivy): "
  jq '.components | length' "trivy_sbom_cyclonedx_${NAME}.json"

  echo "🛡️ Running Grype scan..."
  write_html_template
  grype sbom:"syft_sbom_spdx_${NAME}.json" -o template -t html.tmpl > "report_${NAME}.html"
  grype sbom:"syft_sbom_spdx_${NAME}.json" -o table > "report_${NAME}.txt"
  rm html.tmpl

  echo "📄 Writing Python comparison scripts..."

  # SPDX Comparison Python (CORRECTED)
  cat > compare_spdx.py << EOF
import json, os, sys

def load(path):
    with open(path) as f: return json.load(f)

def extract(sbom): return {os.path.basename(p.get("name", "")): p.get("versionInfo", "Unknown") for p in sbom.get("packages", [])}

syft = extract(load(sys.argv[1]))
trivy = extract(load(sys.argv[2]))
all_keys = sorted(set(syft.keys()).union(set(trivy.keys())))
rows = []

for pkg in all_keys:
    s, t = syft.get(pkg, "Not Found"), trivy.get(pkg, "Not Found")
    if s == t and s != "Not Found": status, color = "✅ Match", "#d4edda"
    elif s == "Not Found": status, color = "⚠️ Missing in Syft", "#fff3cd"
    elif t == "Not Found": status, color = "⚠️ Missing in Trivy", "#fff3cd"
    else: status, color = "❌ Version Mismatch", "#f8d7da"
    rows.append((pkg, s, t, status, color))

with open(sys.argv[3], "w") as f:
    f.write("<html><head><style>body{font-family:Arial;padding:2em}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #ccc;padding:8px;text-align:left;}th{background:#eee}</style></head><body><h2>SPDX Comparison</h2><table><tr><th>Package</th><th>Syft</th><th>Trivy</th><th>Status</th></tr>")
    for row in rows:
        f.write(f"<tr style='background-color:{row[4]}'><td>{row[0]}</td><td>{row[1]}</td><td>{row[2]}</td><td>{row[3]}</td></tr>")
    f.write("</table></body></html>")
EOF

  # CycloneDX Comparison Python (CORRECTED)
  cat > compare_cyclonedx.py << EOF
import json, os, sys

def load(path):
    with open(path) as f: return json.load(f)

def extract(sbom): return {os.path.basename(c.get("name", "")): c.get("version", "Unknown") for c in sbom.get("components", [])}
...
with open(sys.argv[3], "w") as f:
    f.write("<html><head><style>body{font-family:Arial;padding:2em}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #ccc;padding:8px;text-align:left;}th{background:#eee}</style></head><body><h2>CycloneDX Comparison</h2><table><tr><th>Package</th><th>Syft</th><th>Trivy</th><th>Status</th></tr>")
    for row in rows:
        f.write(f"<tr style='background-color:{row[4]}'><td>{row[0]}</td><td>{row[1]}</td><td>{row[2]}</td><td>{row[3]}</td></tr>")
    f.write("</table></body></html>")
EOF

  echo "🔍 Running SPDX comparison..."
  python3 compare_spdx.py \
    "syft_sbom_spdx_${NAME}.json" \
    "trivy_sbom_spdx_${NAME}.json" \
    "spdx_comparison_reports/SPDX_SBOM_comparison_report_${NAME}.html"

  echo "🔍 Running CycloneDX comparison..."
  python3 compare_cyclonedx.py \
    "syft_sbom_cyclonedx_${NAME}.json" \
    "trivy_sbom_cyclonedx_${NAME}.json" \
    "cyclonedx_comparison_reports/CycloneDX_SBOM_comparison_report_${NAME}.html"

  echo "✅ SBOM Analysis Complete!"
}
...
while true; do
  ...
  echo "1. Update Syft, Grype, and Trivy"
  echo "2. Run Combined SBOM Analysis (Syft + Trivy + Grype)"
  echo "3. Run Syft & Grype Analysis Only"
  echo "4. Run Trivy Analysis Only"
  echo "5. Compare SPDX SBOMs (Manual)"
  echo "6. Compare CycloneDX SBOMs (Manual)"
  echo "7. Open an HTML Report"
  echo "8. Exit"
  ...
  case "$choice" in
    ...
    2)
      ...
      ;;
    3)
      run_syft_grype_analysis
      ;;
    4)
      run_trivy_analysis
      ;;
    5)
      read -p "Path to syft_sbom_spdx_<name>.json: " syft_file
      read -p "Path to trivy_sbom_spdx_<name>.json: " trivy_file
      read -p "Output report path (e.g., SPDX_report.html): " output_html
      python3 compare_spdx.py "$syft_file" "$trivy_file" "$output_html"
      echo "✅ Report generated: $output_html"
      ;;
    6)
      read -p "Path to syft_sbom_cyclonedx_<name>.json: " syft_file
      read -p "Path to trivy_sbom_cyclonedx_<name>.json: " trivy_file
      read -p "Output report path (e.g., CycloneDX_report.html): " output_html
      python3 compare_cyclonedx.py "$syft_file" "$trivy_file" "$output_html"
      echo "✅ Report generated: $output_html"
      ;;
    7)
      read -p "Enter path to HTML report to open: " html_file
      if command -v xdg-open >/dev/null 2>&1; then
        xdg-open "$html_file"
      elif command -v open >/dev/null 2>&1; then
        open "$html_file"
      else
        echo "❌ Cannot open browser. Please open manually: $html_file"
      fi
      ;;
    8)
      ...
      ;;
    *)
      echo "⚠️ Invalid choice. Please enter a number from 1 to 8."
      ;;
  esac
done
