...
check_update_trivy() {
  if ! command -v trivy >/dev/null 2>&1; then
    ...
    echo "🔍 Trivy current version: $(trivy version | head -1)"
  fi
  echo "⬆️ Updating Trivy to latest version..."
  curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sudo sh -s -- -b /usr/local/bin
  echo "✅ Trivy updated to: $(trivy version | head -1)"
}
...
# === Function: Syft SBOM & Grype Analysis ===
...
EOF
  ...
  # === SBOM Generation ===
  echo "[1] Generating SPDX and CycloneDX SBOMs with Syft..."
  if [[ "$SCAN_MODE" == "image" ]]; then
    syft "$TARGET" -o spdx-json > syft_sbom_spdx.json
    syft "$TARGET" -o cyclonedx-json > syft_sbom_cyclonedx.json
  else
    syft dir:"$TARGET" -o spdx-json > syft_sbom_spdx.json
    syft dir:"$TARGET" -o cyclonedx-json > syft_sbom_cyclonedx.json
  fi

  # === Count Components ===
  echo "[2] Component Count:"
  echo -n "  SPDX Packages: "
  jq '.packages | length' syft_sbom_spdx.json
  echo -n "  CycloneDX Components: "
  jq '.components | length' syft_sbom_cyclonedx.json

  # === Grype Scan ===
  echo "[3] Scanning SBOM with Grype..."
  grype sbom:syft_sbom_spdx.json -o template -t html.tmpl > syft_grype_report.html
  grype sbom:syft_sbom_spdx.json -o table > syft_grype_report.txt
  grype sbom:syft_sbom_spdx.json -o json > syft_grype_report.json

  echo "✅ Syft-Grype analysis completed!"
  echo "📄 HTML report: $OUTPUT_DIR/syft_grype_report.html"
  echo "📄 Text report: $OUTPUT_DIR/syft_grype_report.txt"
  echo "📄 JSON report: $OUTPUT_DIR/syft_grype_report.json"
}

# === Function: Trivy SBOM Generation & Analysis ===
run_trivy_analysis() {
  read -p "Enter path to application folder OR Docker image name: " TARGET
  read -p "Enter path where output should be saved: " OUTPUT_DIR
  ...
  echo "[1] Generating SBOM with Trivy..."
  if [[ "$SCAN_MODE" == "image" ]]; then
    # Generate SBOM
    trivy image --format spdx-json --output trivy_sbom_spdx.json "$TARGET"
    trivy image --format cyclonedx --output trivy_sbom_cyclonedx.json "$TARGET"
    
    # Generate vulnerability reports
    trivy image --format json --output trivy_vuln_report.json "$TARGET"
    trivy image --format table --output trivy_vuln_report.txt "$TARGET"
    
  else
    # Generate SBOM
    trivy fs --format spdx-json --output trivy_sbom_spdx.json "$TARGET"
    trivy fs --format cyclonedx --output trivy_sbom_cyclonedx.json "$TARGET"
    
    # Generate vulnerability reports
    trivy fs --format json --output trivy_vuln_report.json "$TARGET"
    trivy fs --format table --output trivy_vuln_report.txt "$TARGET"
  fi

  echo "[2] Component Count:"
  if [[ -f "trivy_sbom_spdx.json" ]]; then
    echo -n "  SPDX Packages: "
    jq '.packages | length' trivy_sbom_spdx.json 2>/dev/null || echo "N/A"
  fi
  if [[ -f "trivy_sbom_cyclonedx.json" ]]; then
    echo -n "  CycloneDX Components: "
    jq '.components | length' trivy_sbom_cyclonedx.json 2>/dev/null || echo "N/A"
  fi

  echo "✅ Trivy analysis completed!"
  echo "📄 SBOM (SPDX): $OUTPUT_DIR/trivy_sbom_spdx.json"
  echo "📄 SBOM (CycloneDX): $OUTPUT_DIR/trivy_sbom_cyclonedx.json"
  echo "📄 Text report: $OUTPUT_DIR/trivy_vuln_report.txt"
  echo "📄 JSON report: $OUTPUT_DIR/trivy_vuln_report.json"
}

# === Function: Compare Trivy & Syft Reports ===
compare_reports() {
  ...
  if command -v xdg-open >/dev/null; then
    ...
    echo "❌ Cannot detect browser open command. Visit manually: $COMPARISON_URL"
  fi
  
  echo ""
  echo "💡 Comparison Tips:"
  echo "   • Compare the number of vulnerabilities found by each tool"
  echo "   • Check for differences in severity ratings"
  echo "   • Look for unique vulnerabilities found by each scanner"
  echo "   • Compare SBOM component counts"
  echo "   • Analyze false positive rates"
  echo ""
  echo "📁 Make sure you have both Syft-Grype and Trivy reports in the same directory for comparison."
}
...
while true; do
  ...
  echo "1. Update Syft, Grype & Trivy"
  echo "2. Start Syft SBOM Generation & Grype Analysis"
  echo "3. Start Trivy SBOM Generation & Analysis"
  echo "4. Compare Trivy & Syft Reports"
  echo "5. GitHub Profile (naveen-sekhar)"
  echo "6. Exit"
  ...
  case "$choice" in
    ...
    3)
      if ! command -v trivy >/dev/null 2>&1; then
        echo "❌ Trivy is not installed. Please run option 1 to install/update Trivy first."
      else
        run_trivy_analysis
      fi
      ;;
    ...
    6)
      ...
      ;;
    *)
      echo "⚠️ Invalid option. Please enter a number from 1 to 6."
      ;;
  esac
done
